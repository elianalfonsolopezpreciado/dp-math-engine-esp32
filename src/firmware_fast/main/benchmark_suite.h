/*
 * Purpose: Benchmark harness for the Q16.16 firmware with deterministic test
...
#include "cordic.h"
#include "matrix_q16.h"
#include "uart_protocol.h"

namespace BenchmarkSuite {

using FastMathEngine::Matrix;
using FastMathEngine::cordic_sin_cos;
using FastMathEngine::floatToQ;
using FastMathEngine::matmul_q16;
using FastMathEngine::mulQ;
using FastMathEngine::q16_t;
using FastMathEngine::qToFloat;

static constexpr const char* FIRMWARE_NAME = "FastMathEngine_Q1616";
...
inline float matrix_norm_q16(const int32_t* data, size_t count) {
    double sum = 0.0;
    for (size_t i = 0; i < count; ++i) {
        const double value = static_cast<double>(qToFloat(static_cast<q16_t>(data[i])));
        sum += value * value;
    }
...
inline void run_scalar_trig_test(int test_id, const char* category, const char* function_name, float angle) {
    const q16_t theta_q = floatToQ(angle);
    q16_t sin_q = 0;
    q16_t cos_q = 0;

    portDISABLE_INTERRUPTS();
    const uint32_t cycles_start = xthal_get_ccount();
    cordic_sin_cos(theta_q, &sin_q, &cos_q);
    const q16_t result_q = (function_name[0] == 's') ? sin_q : cos_q;
    const uint32_t cycles_end = xthal_get_ccount();
    portENABLE_INTERRUPTS();

    const float result = qToFloat(result_q);
...
inline void run_scalar_mul_test(int test_id, float a, float b) {
    const q16_t aq = floatToQ(a);
    const q16_t bq = floatToQ(b);

    portDISABLE_INTERRUPTS();
    const uint32_t cycles_start = xthal_get_ccount();
    const q16_t result_q = mulQ(aq, bq);
    const uint32_t cycles_end = xthal_get_ccount();
    portENABLE_INTERRUPTS();
...
inline void run_matrix_test(int test_id, uint16_t dim, const char* category, uint32_t* rng_state) {
    ...
    int32_t* qa = static_cast<int32_t*>(heap_caps_malloc(sizeof(int32_t) * count, MALLOC_CAP_8BIT));
    int32_t* qb = static_cast<int32_t*>(heap_caps_malloc(sizeof(int32_t) * count, MALLOC_CAP_8BIT));
    int32_t* qc = static_cast<int32_t*>(heap_caps_malloc(sizeof(int32_t) * count, MALLOC_CAP_8BIT));

    if (raw_a == nullptr || raw_b == nullptr || ref_c == nullptr || qa == nullptr || qb == nullptr || qc == nullptr) {
        free(raw_a);
        free(raw_b);
        free(ref_c);
        free(qa);
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        qa[i] = floatToQ(raw_a[i]);
        qb[i] = floatToQ(raw_b[i]);
    }
    ...
    const float result_norm = matrix_norm_q16(qc, count);
